...
runs `pip check`, runs the test suite, and checkpoints its last completed step
under `output/state/setup/`. Rerunning the same script reuses the compatible
environment and package-download cache. A legacy environment without the
...
runs the CPU-safe tests so setup can finish, but
`deepdocforgery doctor --profile cuda` returns `attention_required` until at
least 16 GiB VRAM is free and the competing process has stopped.
