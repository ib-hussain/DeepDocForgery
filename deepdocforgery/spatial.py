...
The CPU profile uses the native backbone.  The CUDA profile can use a timm
ImageNet-pretrained backbone while retaining the same feature contract.
"""
...
from .frequency import ConvNormAct, ResidualBlock
...
class TimmFeatureBackbone(nn.Module):
    """Expose selected timm feature maps through the native pyramid contract."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        if len(out_indices) != len(level_names):
            raise ValueError("timm out_indices must match level_names")
        try:
            import timm
        except ImportError as error:
            raise RuntimeError(
                "The timm backbone requires: python -m pip install -e '.[cuda]'"
            ) from error
        kwargs: dict[str, Any] = {
            "features_only": True,
            "pretrained": pretrained,
            "out_indices": tuple(int(value) for value in out_indices),
        }
        # Swin otherwise rejects the 512x512 research geometry as non-224.
        try:
        ...
        source_channels = tuple(int(value) for value in self.model.feature_info.channels())
        if len(source_channels) != len(level_names):
            raise ValueError("timm feature count does not match configured levels")
        self.level_names = tuple(str(value) for value in level_names)
        self.out_channels = tuple(int(value) for value in out_channels)
        self.projections = nn.ModuleDict(
            {
                name: ConvNormAct(source, target, kernel_size=1)
                for name, source, target in zip(
                    self.level_names, source_channels, self.out_channels, strict=True
                )
            }
        )
        self.source_channels = source_channels
        pretrained_config = getattr(self.model, "pretrained_cfg", {}) or {}
        mean = tuple(float(value) for value in pretrained_config.get("mean", (0.485, 0.456, 0.406)))
        std = tuple(float(value) for value in pretrained_config.get("std", (0.229, 0.224, 0.225)))
        if len(mean) != 3 or len(std) != 3 or min(std) <= 0:
            raise ValueError("timm preprocessing mean/std must each contain three valid values")
    ...
    def forward(self, rgb: Tensor) -> dict[str, Tensor]:
        normalized = (rgb - self.input_mean.to(rgb.dtype)) / self.input_std.to(rgb.dtype)
        values = list(self.model(normalized))
        outputs: dict[str, Tensor] = {}
        for name, channels, value in zip(
            self.level_names, self.source_channels, values, strict=True
        ):
            if value.ndim != 4:
                raise ValueError("timm feature maps must be rank-four tensors")
            if value.shape[1] != channels and value.shape[-1] == channels:
                value = value.permute(0, 3, 1, 2).contiguous()
            outputs[name] = self.projections[name](value)
        return outputs
...
class SpatialFeaturePyramid(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        vit_backend: str = "native",
        timm_model_name: str = "swin_tiny_patch4_window7_224.ms_in1k",
        timm_pretrained: bool = False,
        timm_image_size: tuple[int, int] = (512, 512),
        timm_out_indices: Sequence[int] = (1, 2, 3),
        timm_gradient_checkpointing: bool = False,
    ) -> None:
        super().__init__()
        if not out_channels or len(out_channels) != len(level_names):
        ...
        elif vit_backend == "timm":
            self.vit = TimmFeatureBackbone(
                out_channels=self.out_channels,
                level_names=self.level_names,
                model_name=timm_model_name,
                pretrained=timm_pretrained,
                image_size=timm_image_size,
                out_indices=timm_out_indices,
                gradient_checkpointing=timm_gradient_checkpointing,
            )
        else:
            raise ValueError("vit.backend must be 'native' or 'timm'")
    ...
    def from_config(
        ...
        return cls(
            ...
            vit_backend=str(vit.get("backend", "native")),
            timm_model_name=str(vit.get("name", "swin_tiny_patch4_window7_224.ms_in1k")),
            timm_pretrained=bool(vit.get("pretrained", False)) and load_pretrained,
            timm_image_size=tuple(int(value) for value in vit.get("image_size", (512, 512))),
            timm_out_indices=tuple(int(value) for value in vit.get("out_indices", (1, 2, 3))),
            timm_gradient_checkpointing=bool(vit.get("gradient_checkpointing", False)),
        )
...
class ArtifactDecouplingLoss(nn.Module):
    ...
    def forward(
        ...
        sample_valid = valid.flatten(1).sum(dim=1) > 0
        zero = logits.sum() * 0.0

        if targets.domain_index is None:
            text_bce = sample_loss[sample_valid].mean() if bool(sample_valid.any()) else zero
            nontext_bce = zero
            domain = None
        else:
            ...
            text_selection = (domain == 0) & sample_valid
            nontext_selection = (domain == 1) & sample_valid
            text_bce = sample_loss[text_selection].mean() if bool(text_selection.any()) else zero
            nontext_bce = (
                sample_loss[nontext_selection].mean() if bool(nontext_selection.any()) else zero
            )
