...
from deepdocforgery.frequency import ConvNormAct, ResidualBlock
...
class SynergyDenoisingDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # The fused pyramid starts at stride 8.  A stride-2 RGB path restores
        # character-scale edge evidence before the prediction heads.
        detail_hidden = max(8, decoder_channels // 2)
        self.detail_encoder = nn.Sequential(
            ConvNormAct(3, detail_hidden, kernel_size=3, stride=2),
            ResidualBlock(detail_hidden),
            ConvNormAct(detail_hidden, decoder_channels, kernel_size=3),
        )
        self.detail_fusion = nn.Sequential(
            ConvNormAct(2 * decoder_channels, decoder_channels, kernel_size=3),
    ...
    def forward(
        ...
        *,
        ...
        if classification_valid is None:
            condition_valid = torch.ones(
                class_context.shape[0], 1, device=class_context.device, dtype=class_context.dtype
            )
        else:
            condition_valid = classification_valid.to(
                device=class_context.device, dtype=class_context.dtype
            ).reshape(-1, 1)
            if condition_valid.shape[0] != class_context.shape[0]:
                raise ValueError("classification_valid must contain one value per image")
        conditioned_context = class_context * condition_valid
        ...
        for name in reversed(self.level_names[:-1]):
            ...
            state = self.refinements[name](lateral, state, mask_logits, conditioned_context)
        ...
        feedback = self.localization_feedback(
            torch.cat((suspicious, background, local_global), dim=1)
        )
        ...
        class_prior = image_logits.sigmoid()[:, :, None, None] * condition_valid[:, :, None, None]
        synergized = state + class_strength * class_prior * class_map

        detail = self.detail_encoder(rgb)
        synergized = F.interpolate(
            synergized, size=detail.shape[-2:], mode="bilinear", align_corners=False
        )
        synergized = self.detail_fusion(torch.cat((synergized, detail), dim=1))
        decoded["detail_s2"] = synergized
