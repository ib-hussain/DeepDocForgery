...
from src.inputLayer.spatialFeature import SpatialBranchOutput, SpatialFeaturePyramid
...
class MultiScaleAttentionFusion(nn.Module):
    ...
    def from_config(
        ...
        *,
        ...
        return cls(
            ...
            fusion_channels=(
                None if configured_channels is None else tuple(configured_channels)
            ),
    ...
    def forward(
        ...
        spatial_features = spatial.features if isinstance(spatial, SpatialBranchOutput) else dict(spatial)
...
class DeepDocForgeryFusionFrontEnd(nn.Module):
    ...
    @classmethod
    def from_config(cls, config: Mapping[str, Any]) -> DeepDocForgeryFusionFrontEnd:
        ...
        spatial = SpatialFeaturePyramid.from_config(spatial_config)
