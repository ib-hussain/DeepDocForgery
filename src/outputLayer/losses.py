...
def _validate_supervision(supervision: SynergySupervision) -> tuple[Tensor, Tensor, Tensor]:
    ...
    return target.clamp(0.0, 1.0), labels.clamp(0.0, 1.0), valid.clamp(0.0, 1.0)
...
class SynergyMultiTaskLoss(nn.Module):
    ...
    def forward(
        ...
        target, labels, valid = _validate_supervision(supervision)
        ...
        mask_bce = _masked_mean(
            F.binary_cross_entropy_with_logits(
                predictions.mask_logits, target, reduction="none"
            ),
            valid,
        )
        mask_dice = _dice_loss(predictions.mask_logits, target, valid)
        image = F.binary_cross_entropy_with_logits(predictions.image_logits, labels)
        ...
        for auxiliary_logits in predictions.auxiliary_mask_logits.values():
            ...
            auxiliary_losses.append(
                _masked_mean(
                    F.binary_cross_entropy_with_logits(
                        auxiliary_logits, auxiliary_target, reduction="none"
                    ),
                    auxiliary_valid,
                )
        ...
        agreement_per_sample = F.binary_cross_entropy(
            mask_presence, labels, reduction="none"
        )
