...
class ExactJPEGDCTReader:
    ...
    def read(self, path: str | Path) -> ExactDCTBatch:
        ...
        result = ExactDCTBatch(coefficients=coefficients, metadata=metadata)
        result.validate()
        return result
...
class DCTFeaturePyramid(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        exact_features = self._encode(exact_coefficients, metadata.qtables)
        consistency_loss = rgb.new_zeros(())
        if compute_consistency:
            fallback_coefficients = self.pixel_dct(rgb, metadata.qtables, metadata.subsampling)
            if fallback_coefficients.shape != exact_coefficients.shape:
                raise ValueError(
                    "Exact and pixel DCT grids differ. Apply the same 8-pixel-aligned crop "
                    "to RGB and exact coefficients; do not independently resize either stream."
                )
            fallback_features = self._encode(fallback_coefficients, metadata.qtables)
            consistency_loss = self.consistency_weight * self._agreement(
                exact_features, fallback_features
            )
        return DCTBranchOutput(exact_features, consistency_loss, "exact_jpeg")
