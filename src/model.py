...
from src.hiddenLayers.fusion import (
    DeepDocForgeryFusionFrontEnd,
    DeepDocForgeryFusionOutput,
)
from src.inputLayer.freqFeatures import ExactDCTBatch, JPEGMetadata
from src.outputLayer.decoder import SynergyDecoderOutput, SynergyDenoisingDecoder
...
class DeepDocForgeryModel(nn.Module):
    ...
    @classmethod
    def from_config(cls, config: Mapping[str, Any]) -> DeepDocForgeryModel:
        front_end = DeepDocForgeryFusionFrontEnd.from_config(config)
    ...
    def forward(
        ...
        *,
        ...
        decoder = self.decoder(front_end.fusion.features, output_size=rgb.shape[-2:])
